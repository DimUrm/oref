 ...
 * Добавляется: Система → Dashboard → Добавить виджет → OrefAlert
 */
?>
registerWidget('oref_alert', {
    ...
    render: function(params) {
        var obj    = params.object_name || 'Alarm';
...
(function(){
  ...
  function update(){
    Promise.all(['Status','Name','CityRu','Img','ShelterTime','LastAlertTS','LastAlarmTime'].map(g))
    .then(function(v){
      var status=v[0],name=v[1],cityRu=v[2],img=v[3],shelter=v[4],lts=v[5],lastAlarm=v[6];
  ...
  function poll(){g('Status').then(function(s){if(s!==last){last=s;update();}});}
  update();
